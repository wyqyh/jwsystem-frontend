<template>
  ...
  <el-form label-width="100px" :model="detailinfo" ref="detailInfoRef" :rules="rules">
    ...
    <h4 class="form-header">细化课表信息</h4>
    <el-row>
      <el-col :span="6">
  ...
  <div v-if="conflict">
    <el-divider content-position="left">细化课表冲突详情</el-divider>
    <el-table :data="conflictCourseDetails">
...
import { addCourseDetail } from '../../../api/course/coursedetail'
...
export default {
  ...
  created() {
    let row = this.$route.query.courseinfo
    this.detailinfo.courseid = row.id
    this.detailinfo.classid = row.classid
    this.detailinfo.classno = row.classno
    Object.assign(this.classinfo, row)
    Object.assign(this.courseinfo, row)
    this.teacIdNames = row.teacIdNames
